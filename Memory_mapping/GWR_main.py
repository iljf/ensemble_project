...
from env import Env
from GWR_memory import GWRMemory
from GWR_test import test
...
if __name__ == '__main__':
    ...
    parser.add_argument('--id', type=str, default='GWR', help='Experiment ID')
    ...
    parser.add_argument('--model_name', type=str, default='DQN', help='Models of Q networks = [DQN, DDQN, NoisyDQN, DuelingDQN, DistributionalDQN]')
    ...
    parser.add_argument('--target-update', type=int, default=int(16000), metavar='τ', help='Number of steps after which to update target network')
    ...
    parser.add_argument('--activation-threshold', type=float, default=0.9, help='max action probability')
    parser.add_argument('--habituation-threshold', type=float, default=0.85, help='max action probability')
    ...
    if args.id == 'GWR':
        wandb.init(project="GWR",
               name=args.model_name + "_r_ " + args.game + "_Seed" + str(args.seed) + "_AT" + str(args.activation_threshold) + "_HT" + str(args.habituation_threshold),
    ...
    val_mem = GWRMemory(args)
    T, done = 0, True
    while T < args.evaluation_size:
        if done:
            state, done = env.reset(), False
        next_state, _, done = env.step(np.random.randint(0, action_space))
        val_mem.append(state, None, None, next_state, done)
        state = next_state
        T += 1
    ...
    if args.evaluate:
        ...
        while T < args.learn_start:
            ...
            mem.append(state, action, reward, next_state, done)  # Append transition to memory
            state = next_state
            T += 1
        ...
        for T in trange(1, args.T_max + 1):
            ...
            if T % args.replay_frequency == 0:
                loss = dqn.GWR_learn(mem, args.model_name)
            ...
            if T % args.evaluation_interval == 0:
                ...
                wandb.log({'eval/reward_mode': reward_mode_[T-1],
                            ...
                            'eval/timestep': T,
                            'eval/Memory': len(mem.nodes),
                            'Q-value/Q-value': avg_Q,
                            'Q-value/batch-loss': loss
                             },step=T)
