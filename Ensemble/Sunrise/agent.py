...
class Agent():
    ...
    def ensemble_learn(self, idxs, states, actions, returns, next_states, nonterminals, weights, masks, weight_Q=None):
        ...
        if weight_Q is None:
            mse_loss = (weights * masks * m_loss).mean()
        else:
            mse_loss = (weight_Q * weights * masks * m_loss).mean()
        ...
        self.optimiser.step()
        return loss.detach().cpu().numpy(), batch_loss.detach().cpu().item(), mse_loss.detach().cpu().item()
