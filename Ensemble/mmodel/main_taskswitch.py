...
def predefined_scheduler_taskswitch(schedule_mode=1, action_prob_set = None, min_max_action_prob = [0.1, 0.9], max_length = 5000000, numstep_task = 500000, debug = False):
        ...
        # randind for num_env
        randind = np.random.randint(0, len(Envs), num_env)
        Envs2out = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--T-max', type=int, default=int(3.5e6), metavar='STEPS', help='Number of training steps (4x number of frames)')
    ...
    reward_mode_, action_probs_, Envs_ = predefined_scheduler_taskswitch(args.scheduler_mode, min_max_action_prob = [args.action_prob_min, args.action_prob_max], numstep_task=5000)
    ...
    if args.evaluate:
        ...
        for T in trange(1, args.T_max + 1):
            ...
            if game_bf != game_af:
                env = Env(args)
                for i in range(args.num_ensemble):
                    dqn_list[i].action_space = env.action_space()
